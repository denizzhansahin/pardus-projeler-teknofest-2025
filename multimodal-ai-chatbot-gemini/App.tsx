...
import { generateContent, fileToBase64, blobToBase64 } from './services/geminiService'; // Added blobToBase64
...
const App: React.FC = () => {
  ...
  const handleSendMessage = useCallback(async (text: string, media?: MediaAttachment) => {
    ...
    if (!process.env.API_KEY) {
        ...
        const systemMessage = createMessageObject("AI çevrimdışı (API anahtarı eksik).", Sender.System, undefined, false, "API Anahtarı yapılandırılmamış");
        addMessageToConversation(currentChatId, systemMessage);
        return;
    }
    ...
    if (media?.file) {
      try {
        if (media.type === MediaType.Image && media.file instanceof File) {
          imageBase64 = await fileToBase64(media.file);
          imageMimeType = media.file.type;
        } else if (media.type === MediaType.Audio && media.file instanceof Blob) {
          audioBase64 = await blobToBase64(media.file); 
          audioMimeType = media.file.type || 'audio/webm'; 
        }
      } catch (error) {
        console.error("Medya dosyası base64'e dönüştürülürken hata oluştu:", error);
        updateMessageInConversation(currentChatId, aiPlaceholderMessage.id, { text: "Üzgünüm, yüklenen medyayı işleyemedim.", error: "Medya işleme hatası." });
        setIsAiLoading(false);
        if (media.url) revokeTrackedUrl(media.url); 
        return;
      }
    }

    const searchKeywords = ["son", "en son", "haber", "güncel", "bugün", "kim kazandı", "hava durumu nedir"];
    const useGoogleSearch = searchKeywords.some(keyword => text.toLowerCase().includes(keyword));
    ...
    if (result.error || !result.text) {
      updateMessageInConversation(currentChatId, aiPlaceholderMessage.id, { text: result.error || "Üzgünüm, yanıt alamadım.", error: result.error || "Yanıt metni yok." });
    } else {
      updateMessageInConversation(currentChatId, aiPlaceholderMessage.id, { text: result.text, groundingSources: result.groundingSources });
    }
    setIsAiLoading(false);
  }, [activeConversationId, handleNewChat, revokeTrackedUrl, addMessageToConversation, updateMessageInConversation, createMessageObject]);
